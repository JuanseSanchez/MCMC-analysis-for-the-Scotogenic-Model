...
class smmcmc:
    ...
    def run_micromegas(self):
        ...
        while i < len(lines):
            ...
            if line.strip().startswith("Masses of odd sector Particles"):
                block = [line]
                i += 1
                # Collect all lines until a blank line (end of masses block)
                while i < len(lines):
                    if lines[i].strip() == "":
                        break
                ...
                while i < len(lines) and lines[i].strip() == "":
                    i += 1
                # 2. Collect the Xf=... Omega h^2=... line
                if i < len(lines) and lines[i].strip().startswith("Xf="):
                    block.append(lines[i])
                    i += 1
                # skip any blank lines
                while i < len(lines) and lines[i].strip() == "":
                    i += 1
                # 3. Collect the channels block
                if i < len(lines) and lines[i].strip().startswith("# Channels which contribute to 1/(omega)"):
                    # header
                    block.append(lines[i])
                    i += 1
                    # relative contributions header
                    if i < len(lines) and lines[i].strip().startswith("# Relative contributions"):
                        block.append(lines[i])
                        i += 1
                                # percentage lines
                    while i < len(lines):
                        if lines[i].strip() == "" or lines[i].strip().startswith("Masses of odd sector Particles"):
                            break
                        block.append(lines[i])
                        i += 1
                info_txt.append("\n".join(block))
        ...
        # Save all blocks to a single txt file
        if info_txt:
            with open("micromegas_info.txt", "a") as infofile:
                infofile.write("\n\n".join(info_txt))
                infofile.write("\n\n")
            ...
            with tqdm(total=self.accepted_points) as pbar:
                ...
                for sample in self.sampler.sample(init_parameters, iterations = self.steps, progress = True, store = False):
                    ...
                    if self.write_only_accepted == False:
                        ...
                        for r in row:
                            t = tuple(r)
                            if t not in unique_points:
                                unique_points.add(t)
                                new_rows.append(r)
                        if new_rows:
                            accepted_samples += len(new_rows)
                            pbar.update(len(new_rows))
                            f.writerows(new_rows)
                            file.flush()
                    else:
                        accepted_samples_array = row[row[:,-1] >= np.exp(self.likelihood_threshold)]
                        new_rows = []
                        for r in accepted_samples_array:
                            t = tuple(r)
                            if t not in unique_points:
                                unique_points.add(t)
                                new_rows.append(r)
